...
def Conv(data, num_filter, kernel, stride=(1,1), pad=(0, 0), dilate=(1,1), name=None, no_bias=True, w=None, b=None, attr=None, num_group=1):
    Convolution = mx.symbol.Convolution
    if w is None:
        conv     = Convolution(data=data, num_filter=num_filter, num_group=num_group, kernel=kernel, pad=pad, stride=stride, dilate=dilate, name=('%s__conv' %name), no_bias=no_bias, attr=attr)
    else:
        if b is None:
            conv = Convolution(data=data, num_filter=num_filter, num_group=num_group, kernel=kernel, pad=pad, stride=stride, dilate=dilate, name=('%s__conv' %name), no_bias=no_bias, weight=w, attr=attr)
        else:
            conv = Convolution(data=data, num_filter=num_filter, num_group=num_group, kernel=kernel, pad=pad, stride=stride, dilate=dilate, name=('%s__conv' %name), no_bias=False, bias=b, weight=w, attr=attr)
    return conv
...
def AC_Conv(   data, num_filter,  kernel, pad, stride=(1,1), dilate=(1,1),name=None, w=None, b=None, no_bias=True, attr=None, num_group=1):
    ac     = AC(     data=data,   name=('%s__ac' % name))
    ac_cov = Conv(   data=ac,     num_filter=num_filter, num_group=num_group, kernel=kernel, pad=pad, stride=stride, dilate=dilate, name=name, w=w, b=b, no_bias=no_bias, attr=attr)
    return ac_cov

def BN_AC_Conv(data, num_filter,  kernel, pad, stride=(1,1), dilate=(1,1), name=None, w=None, b=None, no_bias=True, attr=None, num_group=1):
    bn     = BN(     data=data,   name=('%s__bn' % name))
    ba_cov = AC_Conv(data=bn,     num_filter=num_filter, num_group=num_group, kernel=kernel, pad=pad, stride=stride, dilate=dilate, name=name, w=w, b=b, no_bias=no_bias, attr=attr)
    return ba_cov
