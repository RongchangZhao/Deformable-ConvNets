...
def add_data_args(parser):
    ...
    data.add_argument('--train-image-root2', type=str)
    ...
    data.add_argument('--rgb-mean', type=str, default='', help='a tuple of size 3 for the mean rgb')
...
class StIter(mx.io.DataIter):
    def __init__(self, iter1, iter2, data_shape, batch_size, size1, ctx_num, data_name='data', label_name='softmax_label'):
      self.iter1 = iter1
      self.iter2 = iter2
      ...
      self.size1 = size1
      self.size2 = batch_size - size1
      self.per_size1 = int(size1/ctx_num)
      self.per_size2 = int( self.size2/ctx_num )
      self.ctx_num = ctx_num
      self.provide_data = [(self.data_name, (batch_size,) + self.data_shape)]
      self.provide_label = [(self.label_name, (batch_size,) )]

    def num_samples(self):
      return self.iter1.num_samples()

    def part_slice(self, d, a, b):
      _d = mx.ndarray.slice_axis(d, axis=0, begin=a, end=b)
      return _d
...
def get_rec_iter(args, kv=None):
    ...
    if args.size1<args.batch_size:
      train2 = ImageIter2(
          path_root	          = args.image_root, 
          path_imglist        = args.train_lst2,
          #smooth_param        = './data/confusion_matrix.npy,3,0.1,0.1',
          balance             = 1000,
          data_shape          = image_shape,
          batch_size          = args.batch_size-args.size1,
          #resize              = train_resize,
          rand_crop           = True,
          rand_resize         = True,
          rand_mirror         = True,
          ...
          num_parts           = nworker,
          part_index          = rank)
      train = StIter(train1, train2, image_shape, args.batch_size, args.size1, args.ctx_num)
    else:
      train = train1
    if args.val_lst is not None:
      val1 = ImageIter2(
          ...
          batch_size          = args.size1,
          data_shape          =  image_shape,
          resize		      = image_shape[1]+int(image_shape[1]/28), 
          rand_crop       = False,
          rand_resize     = False,
          rand_mirror     = False,
          rgb_mean            = rgb_mean,
      ...
      if args.size1<args.batch_size:
        val2 = ImageIter2(
            ...
            num_parts       = nworker,
            part_index      = rank)
        val = StIter(val1, val2, image_shape, args.batch_size, args.size1, args.ctx_num)
      else:
        val = val1
    else:
      val = None
