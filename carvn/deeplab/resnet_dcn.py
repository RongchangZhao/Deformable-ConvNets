...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck=True, bn_mom=0.9, workspace=256, memonger=False, dilate=(1,1), use_deformable=False):
...
def resnet_conv(data, units, num_stages, filter_list, num_classes, image_shape, bottle_neck=True, bn_mom=0.9, workspace=256, dtype='float32', is_seg = False):
    ...
    for i in range(num_stages):
        if not is_seg:
          _dilate = 1
          _use_deformable = False if i<3 else True
        ...
        body = residual_unit(body, filter_list[i+1], (stride, stride), False,
                             ...
                             memonger=memonger, dilate=(_dilate,_dilate), use_deformable = _use_deformable)
        for j in range(units[i]-1):
            mg_index = (j+1)%len(multi_grid)
            mg = multi_grid[mg_index]
            body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i + 1, j + 2),
                                 bottle_neck=bottle_neck, workspace=workspace, memonger=memonger, dilate=(_dilate*mg,_dilate*mg), use_deformable=_use_deformable)
    return body
...
def get_seg_score(num_classes, num_layers, image_shape, conv_workspace=256, dtype='float32', ignore_label = None, **kwargs):
    ...
    for i in xrange(1):
      ...
      if i==0:
        _score = mx.symbol.Convolution(data=relu_fc6, kernel=(1, 1), pad=(0, 0), num_filter=num_classes, 
            name="fc6_%d_segscore" % i, bias = _bias, weight = _weight, workspace=conv_workspace)
      else:
        pad = (i*6, i*6)
        dilate = pad
        _score = mx.symbol.Convolution(data=relu_fc6, kernel=(3, 3), pad=pad, dilate=dilate, num_filter=num_classes, 
            name="fc6_%d_segscore" % i, bias = _bias, weight = _weight, workspace=conv_workspace)
      scores.append(_score)
    score = mx.symbol.add_n(*scores, name='fc6_score')
    ...
    #gpool = mx.symbol.Convolution(data=gpool, kernel=(1, 1), pad=(0, 0), num_filter=num_classes, 
    #    name="fc6_gpool_segscore", bias = _bias, weight = _weight, workspace=conv_workspace)
    #score = mx.symbol.broadcast_add(score, gpool, name='fc6_score2')
...
def init_weights(sym, data_shape_dict, arg_params, aux_params):
    ...
    if 'upsampling_weight' in arg_shape_dict:
      print('initializing upsampling_weight')
      arg_params['upsampling_weight'] = mx.nd.zeros(shape=arg_shape_dict['upsampling_weight'])
